...
#read csv
state_1_ehist_data <- read_csv("ehist_Atlantic.csv")

#clean_names
state_1_ehist_data_clean_names <- clean_names(state_1_ehist_data)

#select columns
state_1_ehist_selected <- select(state_1_ehist_data_clean_names, voter_id, voter_legacy_id, voter_status, 
...
#typeof(state_1_ehist_selected$voter_registration_date)

#class(state_1_ehist_selected$voter_registration_date)

#change type
state_1_ehist_selected[, c(9, 12)] <- lapply(state_1_ehist_selected[, c(9, 12)], as.numeric)

#read csv
state_1_vlist_data <- read_csv("vlist_Atlantic.csv")

#clean col names
state_1_vlist_data_clean_names <- clean_names(state_1_vlist_data)

#select cols
state_1_vlist_selected <- select(state_1_vlist_data_clean_names, display_id, reg_date, status, first, last)

#check col object type and class
#typeof(state_1_vlist_selected$reg_date)

#class(state_1_vlist_selected$reg_date)

#convert datetime to date
system.time(state_1_vlist_selected$reg_date <- as.Date(state_1_vlist_selected$reg_date))


#to uppercase for join

state_1_vlist_upper <- state_1_vlist_selected %>% 
  mutate(across(where(is.character), toupper)) 

state_1_ehist_upper <- state_1_ehist_selected %>% 
  mutate(across(where(is.character), toupper))

#class(state_1_vlist_upper$reg_date)


#join vlist and ehist
combined <- state_1_vlist_upper %>% full_join(state_1_ehist_upper, by = c("display_id" = "voter_id"))
...
state_1_corrections_data <- read_csv("OPRA #20334 - Release data.csv")


#clean_names
state_1_corrections_data_clean_names <- clean_names(state_1_corrections_data)

#to upper

state_1_corrections_upper <- state_1_corrections_data_clean_names %>% 
  mutate(across(where(is.character), toupper))

#convert characters to dates

state_1_corrections_upper$released <- mdy(state_1_corrections_upper$released)
state_1_corrections_upper$dob <- mdy(state_1_corrections_upper$dob)

#check col object type and class
#typeof(state_1_corrections_upper$dob)

#class(state_1_corrections_upper$dob)



combined_w_corrections <- state_1_corrections_upper %>% full_join(combined, by = c("last", "first", "dob" = "voter_dob"))
...
test3 <- state_1_corrections_upper %>%  filter(is.na(sbi) == TRUE)
